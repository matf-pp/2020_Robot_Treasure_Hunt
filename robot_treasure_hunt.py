    ...
    for i in lista_prepreka:
        if i.x == x and i.y == y:
            if igrac.zivoti == 1:
                ...
                pygame.display.flip()
                Tk().wm_withdraw() #to hide the main window
                messagebox.showinfo('OBSTACLE','You lost one life')
...
def krajnjiEkran():
    ...
    winningSound = mixer.Sound("win_sound.wav")
    winningSound.play()
...
def krajnjiEkran1():
    ...
    losingSound = mixer.Sound("over.wav")
    losingSound.play()
